joincompt=function(idlist,g,t=100){
  
  subdist=igraph::distances(g,v=idlist,to=idlist)
  closure=igraph::graph_from_adjacency_matrix(subdist,mode="undirected",weighted=T)
  clos_mst=igraph::mst(closure,algorithm="prim")
  clos_mst_table=igraph::as_data_frame(clos_mst,what="edges")
  vnames=unlist(igraph::vertex_attr(g,index=V(g)))
  id1=vector()
  id2=vector()
  all2add=list()
  allvec=vector()
  ...
  for (i in 1:nrow(clos_mst_table)){
    ...
    if (clos_mst_table[i,3]>1){
      if (clos_mst_table[i,3]<=t){
        sp=igraph::all_shortest_paths(g,from=id1[i],to=id2[i])$res
        temp2add=list()
  ...
  subg=igraph::induced_subgraph(g,subnodes)
